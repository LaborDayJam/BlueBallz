...
namespace Splyt 
{
	...
	public class Core 
	{
        ...
        private const string SDK_VERSION = "4.0.4";
		...
		public static void init(InitParams initParams, Callback cb) 
		{
            ...
            if (Error.Success == error) 
			{
				...
				#if UNITY_IPHONE && !UNITY_EDITOR
				splyt_core_init(initParams.customerId, initParams.userInfo.entityId, userProperties, initParams.deviceInfo.entityId, deviceProperties, initParams.requestTimeout, initParams.host, initParams.logEnabled, SDK_NAME_PREFIX, SDK_VERSION, HUB_OBJECT);
				#elif UNITY_ANDROID && !UNITY_EDITOR
				callNativeAsync("splyt_core_init", new object[] {_getContext(), initParams.customerId, initParams.userInfo.entityId, userProperties, initParams.deviceInfo.entityId, deviceProperties, initParams.requestTimeout, initParams.host, initParams.logEnabled, SDK_NAME_PREFIX, SDK_VERSION, HUB_OBJECT});
				#else
				...
				// Unfortunatley, there is no good way to determine that at run time within the plugin.
				#if UNITY_WEBPLAYER
		...
		[DllImport("__Internal")]
		private static extern void splyt_core_init(string customerId, string userId, string userProperties, string deviceId, string deviceProperties, int reqTimeout, string host, bool logEnabled, string sdkNamePre, string sdkVersion, string callbackObj);
		[DllImport("__Internal")]
